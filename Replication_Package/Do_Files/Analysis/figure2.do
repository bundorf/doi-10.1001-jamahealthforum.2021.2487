...
twoway (scatter type anyins_b if counter==1, yscale(rev) msym(o) mcol(black*0.75)) || ///
  ...
  xline(0, lcolor(cranberry*0.5)) ysize(5) ///
  xlabel(-0.010(0.005)0.010, nogrid) xlabel(, nogrid) ///
  legend(order(1 "Any Insurance" 3 "Emp. Spons. Ins." 5 "Other Private Insurance" 7 "Medicaid" 9 "Other Types of Public Coverage") pos(6) cols(2) size(2.5)) ///
  ylabel( 1 "All" 2 "Non-Expansion" 3 "Expansion", nogrid) name(figure2_phase1, replace) ///
...
twoway (scatter type anyins_b if counter==1, yscale(rev) msym(o) mcol(black*0.75)) || ///
  ...
  xline(0, lcolor(cranberry*0.5)) ysize(5) ///
  xlabel(-0.010(0.005)0.010, nogrid) xlabel(, nogrid) ///
  ylabel( 1 "All" 2 "Non-Expansion" 3 "Expansion", nogrid) ///
  legend(off) ///
  b1title("Weekly Percentage Point Change", size(2.5)) b2title("Fall/Winter") name(figure2_phase23, replace)

****************
*** COMBINED ***
****************

grc1leg figure2_phase1 figure2_phase23,  legendfrom(figure2_phase1) col(2) ysize(3)

* Save as PNG 
graph export "$results/figure2.png", as(png) name("Graph") width(4000) replace

* Save as EPS
graph export "$results/figure2.eps", as(eps) name("Graph") replace
